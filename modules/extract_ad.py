...
def execute_ssh_command(ip, username, password, command):
    ...
    try:
        ...
        output = _decode(stdout.read())
        error = _decode(stderr.read())
...
def detect_active_directory(ip, username, password):
    # PowerShell command to check AD-related services (forced via powershell, status in English)
    command = "powershell -Command \"(Get-Service -Name 'NTDS').Status.ToString()\""
...
def extract_ad_tree(ip, username, password):
    ...
    try:
        ...
        # PowerShell command to get AD tree (forced via powershell)
        command = f"powershell -Command \"Get-ADObject -Filter * -SearchBase '{search_base}' -SearchScope Subtree | Format-Table -Wrap -AutoSize\""
        result = execute_ssh_command(ip, username, password, command)
